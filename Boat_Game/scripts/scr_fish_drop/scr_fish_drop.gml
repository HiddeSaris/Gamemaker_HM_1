...
function scr_fish_drop(probability, table){
...
if (probability > 0 && table != noone)
{
	if (irandom_range(1, 100) <= probability)
	{
		...
		repeat(_num_of_fish)
		{
			...
			_chance_sum += obj_loot_table.fish_catalogue[drop_chance, _drop_table_array[_cur_fish]]
		...
		var _drop_item_object_index = noone;
		
		for (var _i = 0; (_i <= _num_of_fish && _drop_item_object_index == noone); _i++)
		{
			_verify_sum_begin = _verify_sum_end;
			_verify_sum_end += obj_loot_table.fish_catalogue[drop_chance, _drop_table_array[_i]];
			
			if (_drop >= (1 + _verify_sum_begin) && _drop <= _verify_sum_end)
			{
				_drop_item_object_index = obj_loot_table.fish_catalogue[drop_object, _drop_table_array[_i]];
			}
		}
		return _drop_item_object_index
	}
}
}